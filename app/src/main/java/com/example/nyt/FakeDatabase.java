...
public class FakeDatabase {
    ...
    static Article getJsonArticleById(long articleID) {
        return articlesFromJson.get(articleID);
    }
    ...
    private static final HashMap<Long, Article> articlesFromJson = new HashMap<>();

    static void addDataToFakeDatabaseFromJson(ArrayList<Article> articleArrayList) {
        for(int i = 0; i < articleArrayList.size(); i++) {
            Article currentArticle = articleArrayList.get(i);
            articlesFromJson.put(currentArticle.getArticleID(), currentArticle);
        }
    }

    static ArrayList<Article> getAllJsonArticles() {
        return new ArrayList<Article>((List) Arrays.asList(articlesFromJson.values().toArray()));
    }
